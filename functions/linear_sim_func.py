...
def sim_linear(N, N_test, p, error_sd, L = 0.925, level= None, use_true_contour = False, n_boot = 500, ridge_penal = None, MC = False, smoothed = False, residual_boot = False, CV_boot = False, beta = None, X_test = None,  return_range = False):
    if MC:
        mean_boot_l = []
        if beta is None:
    ...
    #import pdb; pdb.set_trace()
    _, L, U, contain, contain_scb, contain_CS_scb, L1, L2, U1, U2, n_points, range_v,inner_points_num,outer_points_num,true_set_points_num = confidence_set_func.prediction_confidence_set(L, level, mean, se, mean_boot_l, mean_test_true = mean_test_true, use_true_contour = use_true_contour, MC = MC)
    p = X_test.shape[1]
    N_test = X_test.shape[0]
    if return_range:
...
def safe_sim_linear(N, N_test, p, error_sd, L = 0.925, level= None, use_true_contour = False, n_boot = 500, ridge_penal = None, MC = False, smoothed = False, residual_boot = False, CV_boot = False, beta = None, X_test = None,  return_range = False):
    try:
        return sim_linear(N, N_test, p, error_sd, L, level, use_true_contour, n_boot, ridge_penal, MC, smoothed,residual_boot, CV_boot, beta, X_test,  return_range)
    except:
