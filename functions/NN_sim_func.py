...
def sim_NN(N, N_test, p, error_sd, transform_func, 
           ...
           L = 0.925, level= 0, use_true_contour = False, n_boot = 500, MC = False, second_stage = False, center_G = False,center_pred = False, 
           beta = None, X_test = None,  return_range = False, batchnorm_ind = False):
    '''Function for neural network simulation
    
    '''
    
    X, y, X_test, y_test, mean_test_true, beta= generate_NN_data(N = N, N_test = N_test, p = p, transform_func = transform_func, error_sd = error_sd, if_tensor = True)
    if MC:
        mean_boot_l = []
        for i in range(n_boot):
            X_new, y_new, _, _,_ = generate_NN_data(N = N, N_test = N_test, p = p, transform_func = transform_func, beta = beta, error_sd = error_sd, if_tensor = True)
    ...
    if return_range:
        return contain, contain_scb, contain_CS_scb, L, U, L1, L2, U1, U2, n_points, mae, range_v, inner_points_num,outer_points_num,true_set_points_num
    else:
